...
export default function LabPage() {
  ...
  const [requestFormData, setRequestFormData] = useState<CreateLabRequestDto>({
    visitId: 0,
    patientId: 0,
    doctorId: 0,
    notes: '',
  ...
  const [currentItem, setCurrentItem] = useState<CreateLabRequestItemDto>({
    testId: 0,
    notes: '',
  });
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const [testsData, requestsData, patientsData, doctorsData, visitsData] = await Promise.all([
        labTestsApi.getAll(),
  ...
  const handleRequestSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await labRequestsApi.create(requestFormData);
  ...
  const handleVisitChange = (visitId: number) => {
  ...
  return (
    <div className="p-8">
      ...
      {/* Request Modal - Similar to Prescriptions */}
      {showRequestModal && (
        <div className="fixed inset-0 bg-black/50 bg-opacity-50 flex items-center justify-center z-50 p-4">
            ...
            <form onSubmit={handleRequestSubmit} className="p-6 space-y-6">
              <div>
                ...
                <select
                  ...
                  onChange={(e) => handleVisitChange(parseInt(e.target.value))}
                  className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none"
                >
                  ...
                  {visits&&visits.map((visit) => {
                    ...
                    return (
                      <option key={visit.id} value={visit.id}>
                        زيارة #{visit.id} - {patient?.fullName} - {doctor?.user?.fullName}
                      </option>
                    );
                  })}
                </select>
              ...
              {requestFormData.visitId > 0 && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-gray-50 p-4 rounded-lg">
                  <div>
                    <span className="text-sm text-gray-600">المريض: </span>
                    <span className="text-sm font-semibold text-gray-900">
                      {patients.find((p) => p.id === requestFormData.patientId)?.fullName}
                    </span>
                  </div>
                  <div>
                    <span className="text-sm text-gray-600">الطبيب: </span>
                    <span className="text-sm font-semibold text-gray-900">
                      {doctors.find((d) => d.id === requestFormData.doctorId)?.user?.fullName}
                    </span>
                  </div>
                </div>
              )}
